...
> [!IMPORTANT]
> Repository presence is not publication approval. The current system and KPI-observation candidates are drafts. Only validated, reviewed, verified, public-classified, non-personal, explicitly display-approved content can enter the public build.

<a id="repository-status"></a>
## Repository Status

- **Model version:** `1.0.0`, declared in [`pyproject.toml`](pyproject.toml).
- **Current structured content:** six draft digital-system records, 30 active KPI definitions, nine draft KPI observations, 11 active portfolio-dashboard configurations, one draft transformation story, and no authored FAQs.
...
An approved translation needs evidence from an approved source or the appropriate human review. Amharic validation specifically rejects content that has neither approved-source evidence nor approval by `human-amharic-reviewer`. Approved Afaan Oromo requires corresponding valid approval evidence. Draft/unapproved translations are excluded from avatar facts.
