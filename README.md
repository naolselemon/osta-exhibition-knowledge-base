...
Chatbot integration is documented in [the release guide](docs/chatbot-release-guide.md).
`python -m scripts.chatbot_release coverage` reports bilingual authoring gaps;
`python -m scripts.chatbot_release build --output releases` produces a validated,
versioned application bundle in addition to the existing four-artifact build.
The chatbot application remains a separate repository. This checkout intentionally
leaves the authorized M-Mesob source PDF absent; a narrowly scoped validator
exception allows the organization-approved M-MESOB records and the approved
Abbooti Kallaqaa video records to build without creating substitute evidence.
See the [implementation handoff](docs/implementation-status.md) for completed work,
verification results, and remaining deployment prerequisites.
...
> [!IMPORTANT]
> Repository presence is not publication approval. The eight exhibition and system records added outside the ETEX collection retain recorded organizational-leadership and language-review authorization. The 30 ETEX 2025 systems, their 33 non-conflicting successor KPI observations, seven transformation stories, and seven FAQs are published per the explicit publication request recorded in this PR; the conflicting E-Land services observation remains draft. Only validated, reviewed, verified, public-classified, non-personal, explicitly display-approved content can enter the public build.

<a id="repository-status"></a>
## Repository Status

- **Model version:** `1.0.0`, declared in [`pyproject.toml`](pyproject.toml).
- **Current structured content:** 39 published digital-system records, 45 active KPI definitions, nine legacy draft observations plus nine published successor observations, three published M-MESOB successor observations, 33 published ETEX 2025 observations and one conflicting ETEX 2025 draft observation, 11 active portfolio-dashboard configurations, seven published ETEX 2025 transformation stories plus one draft legacy story, seven approved published ETEX 2025 FAQs, and 108 generated published FAQ records with native-speaker-approved Afaan Oromo and Amharic translations, including six approved Abbooti Kallaqaa FAQs.
...
- **Automation:** a manual knowledge-release workflow is defined under `.github/workflows/chatbot-release.yml`. Its private runner and protected publisher environment still require administrator configuration. No Makefile, dedicated lint command, or `osta-kb` executable exists.
- **Repository hosting:** Git repository with `main` as the default branch, hosted privately at [`naolselemon/osta-exhibition-knowledge-base`](https://github.com/naolselemon/osta-exhibition-knowledge-base). PowerPoint and PDF source presentations are Git-ignored. CODEOWNERS, pull-request templates, and issue forms are not configured; the manual release workflow has not yet run remotely.

| Term | Current meaning |
| --- | --- |
| Review content | A source record marked `draft`, `needs-review`, internal, not approved for display, or lacking required language approval. It remains review material even if its publication status is `published`. |
...
| [`.github/`](#folder-github) | Manual reviewed-release workflow | Repository administrators | Builds sanitized releases on an authorized private runner | [Guide](#folder-github) |
...
**Purpose.** Defines the manual chatbot knowledge-release workflow. Pull-request templates, issue forms, and CODEOWNERS remain future work.

**Who normally edits it.** Repository administrators and technical maintainers.

**What belongs here.** Hosting workflows and their documented runner/publication prerequisites.
...
**Typical contributor actions.** Configure the `osta-evidence` private runner and protected `publisher` environment; trigger builds of reviewed commit hashes.
...
**Do not.** Do not expose evidence to untrusted pull-request jobs or claim remote checks are enabled merely because the workflow file exists.

**Related templates and guides.** See the [chatbot release guide](docs/chatbot-release-guide.md), [Git and Pull Requests](#git-pull-request), and [`contributing.md`](contributing.md).
...
Current translation maps use `om` for Afaan Oromo and `am` for Amharic. Downstream applications map them to `om-ET` and `am-ET`. Digital-system translations contain `official_name`, `purpose`, and approval metadata. FAQ translations specify `question`, `answer`, approval metadata, and optional reviewed `kpi_unit_labels`. Run the chatbot coverage command for current approval and authoring gaps.
...
6. Keep it in review until fact, KPI, language, privacy, and publisher review is complete. A published record with `workflow.state: needs-review` is still excluded from public/chatbot bundles; the current generated FAQ set has completed this gate.
...
1. The content author adds text only from an approved source or human translator and leaves it unapproved. Generated bilingual FAQs in `content/faqs/visitor-question-drafts.yml` followed this path and now carry recorded native-speaker approval.
...
6. Run validation and inspect [`avatar-facts.json`](dist/avatar-facts.json); unapproved content must remain absent. For a generated FAQ, both translation approvals must use the correct native-language reviewer role and the FAQ workflow state must be `approved` only after review.
...
This repository uses Git with `main` as its default branch and is hosted privately on [GitHub](https://github.com/naolselemon/osta-exhibition-knowledge-base). A manual release workflow is defined; CODEOWNERS, pull-request templates, issue forms, and branch protection still need administrator setup. Use the following collaborator workflow:
