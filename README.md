...
> [!IMPORTANT]
> Repository presence is not publication approval. The six systems and their successor KPI observations are marked published under recorded organizational-leadership authorization; the Oromo and Amharic translations remain drafts until language review. Only validated, reviewed, verified, public-classified, non-personal, explicitly display-approved content can enter the public build.

<a id="repository-status"></a>
## Repository Status

- **Model version:** `1.0.0`, declared in [`pyproject.toml`](pyproject.toml).
- **Current structured content:** six published digital-system records, 30 active KPI definitions, nine draft KPI observations plus nine published successor observations, 11 active portfolio-dashboard configurations, one draft transformation story, and no authored FAQs. Each published system has Oromo and Amharic translation drafts awaiting language review.
...
- **Repository hosting:** Git repository with `main` as the default branch, hosted privately at [`naolselemon/osta-exhibition-knowledge-base`](https://github.com/naolselemon/osta-exhibition-knowledge-base). PowerPoint source presentations are explicitly Git-ignored and are not hosted there. No CODEOWNERS file, pull-request template, issue forms, or GitHub workflow currently exists, so no workflow badge is shown.
...
- a tracked directory contract for authorized source evidence supplied locally; PowerPoint binaries are Git-ignored and absent from GitHub;
...
```mermaid
flowchart TD
    A[Authorized PowerPoint source files] --> B[Inline source_refs with exact slide locators]
...
| [`source-materials/presentations/`](#folder-source-presentations) | Local, Git-ignored PowerPoint sources | Source curators | Provides evidence to authorized local reviewers without tracking binaries | [Guide](#folder-source-presentations) |
...
**Purpose.** [`source-materials/presentations/`](source-materials/presentations/) is the expected local location for the PowerPoint evidence cited by current records. The `.pptx` files are excluded by [`.gitignore`](.gitignore) and are not present in GitHub history after the source-binary purge.

**Who normally edits it.** Authorized source curators only.

**What belongs here.** Authorized original presentation binaries kept only in approved local workspaces. The tracked `.gitkeep` preserves the directory without publishing a presentation.

**Why this folder is necessary.** Current system, observation, and story drafts cite presentation slide locators.

**Typical contributor actions.** Obtain and preserve the authorized file locally, then cite its slide number and—when needed—shape or visual region in the free-text `locator`.

**Required relationships.** Inline source paths and labels must match the authorized local copy. A fresh clone must obtain the referenced presentations through an approved channel before source-existence validation can pass.
...
3. Locate exact evidence. For current PowerPoint files, use a slide number and add a shape/visual-region description when layout matters.
...
Current authored records cite locally supplied presentation binaries. Any authorized source must use a locator precise enough for a second reviewer with access to the same source to find the evidence:
...
Current translation maps use `om` for Afaan Oromo and `am` for Amharic. Downstream applications should map these to the locale tags `om-ET` and `am-ET`. Current digital-system translations contain `official_name`, `purpose`, and approval metadata; FAQ translation shape is not yet specialized. No approved translations are currently authored.

Translations may be stored as `draft` or `needs-review` while being prepared, but they are excluded from public/avatar output. An approved translation needs evidence from an approved source or the appropriate human review. Amharic approval specifically requires an approved source or `status: approved`, `human_reviewed: true`, and `reviewer_role: human-amharic-reviewer`; approved Afaan Oromo follows the equivalent approved-source or human-review rule.
